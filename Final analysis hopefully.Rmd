...
PPCRdata %>% dplyr::select(age,bmi,sex,pain,p_intensity,sleepquality,
                           ...
                           lungdisease) %>% 
        tbl_summary( tatistic = list(all_continuous() ~ "{mean} ({sd})",
                                    all_categorical() ~ "{n} ({p}%)")) %>%
...
If we refer to the analysis of the unadjusted model, the results are the same for the *sleepquality* variable. In summary, there is a statistically significant **linear** association between <u>increasing</u> *sleepquality* values and <u>decreasing</u> the probability of *pain* being present (due to the less than 1 Odds Ratio). There is also positive [jerk](https://en.wikipedia.org/wiki/Jerk_(physics)) in the association between these two variables due to *sleepquality.C* being statistically significant and positive.
...
model6 <- polr (data = PPCRdata, formula = sleepquality ~ pain, Hess = TRUE)
summary(model6)
